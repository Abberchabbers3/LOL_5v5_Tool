...
class RankHandler:
    ...
    def rank_to_num(self, rank: str):
        rank = rank.split(" ")
        if len(rank) > 1:
            return self.rank_to_points[rank[0]] + (4-int(rank[1]))
        return self.rank_to_points[rank[0]]

    def __str__(self):
        output = ""
        for player, score in self.player_scores.items():
            output += f"{player}: \n"
            output += f"{score} which is about {self.score_to_str(score)} \n"
        return output

    def score_to_str(self, score):
        tier = score // 4 * 4
        division = 4 - int(score % 4)
        return f"{self.points_to_rank[tier]} {division}" if division else self.points_to_rank[tier]
